...
def load_trainers():
    ...
    print(f"[LOAD_TRAINERS] محاولة تحميل الملف من المسار: {TRAINERS_FILE}")
    if os.path.exists(TRAINERS_FILE):
        with open(TRAINERS_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
            print(f"[LOAD_TRAINERS] محتوى الملف: {data}")
            return data
    print(f"[LOAD_TRAINERS] الملف غير موجود في المسار: {TRAINERS_FILE}")
    return {"trainers": []}
...
def verify_trainer(username, password):
    """التحقق من صحة بيانات تسجيل الدخول مع تحقق من صحة الملف"""
    data = load_trainers()
    print(f"[VERIFY_TRAINER] محاولة تحقق: username='{username}', password='{password}'")
    # تحقق أن البيانات كائن ويحتوي على مفتاح trainers وقيمته قائمة
    if not isinstance(data, dict) or 'trainers' not in data or not isinstance(data['trainers'], list):
        print(f"[VERIFY_TRAINER ERROR] ملف trainers.json غير صحيح أو تالف: {data}")
        return None
    for trainer in data['trainers']:
        print(f"[VERIFY_TRAINER] مقارنة مع: username='{trainer.get('username')}', password='{trainer.get('password')}'")
        if isinstance(trainer, dict) and trainer.get('username') == username and trainer.get('password') == password:
            print(f"[VERIFY_TRAINER] تطابق مع المدرب: {trainer}")
            return trainer
    print(f"[VERIFY_TRAINER] لم يتم العثور على مدرب بالاسم: {username}")
    return None
...
def login():
    """نقطة نهائية لتسجيل الدخول مع تسجيل الأخطاء بشكل مفصل"""
    try:
        data = request.json
        if not data:
            print("[LOGIN ERROR] لم يتم إرسال بيانات JSON صحيحة من العميل.")
            return jsonify({'success': False, 'message': 'لم يتم إرسال بيانات صحيحة.'}), 400

        username = data.get('username', '').strip()
        password = data.get('password', '').strip()

        if not username or not password:
            print(f"[LOGIN ERROR] اسم المستخدم أو كلمة المرور فارغ. البيانات المستلمة: {data}")
        ...
        if trainer:
            ...
            if trainer.get('name') == 'نورا صباغ':
                role = 'admin'
                trainer['role'] = 'admin'
            print(f"[LOGIN SUCCESS] تسجيل دخول ناجح للمستخدم: {username} بدور: {role}")
            return jsonify({
                'success': True, 
                'trainer': {
                    'id': trainer['id'],
            ...
            print(f"[LOGIN ERROR] اسم المستخدم أو كلمة المرور غير صحيحة. اسم المستخدم: {username}")
            return jsonify({'success': False, 'message': 'اسم المستخدم أو كلمة المرور غير صحيحة'}), 401
    except Exception as e:
        import traceback
        print(f"[LOGIN EXCEPTION] خطأ غير متوقع: {str(e)}")
        print(traceback.format_exc())
        return jsonify({'success': False, 'message': f'خطأ داخلي في السيرفر: {str(e)}'}), 500
