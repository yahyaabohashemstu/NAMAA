...
function updateTotalQuestionsCount() {
  ...
  if (
    trueFalseCount < 0 || trueFalseCount > 100 ||
    multipleChoiceCount < 0 || multipleChoiceCount > 100 ||
    classicCount < 0 || classicCount > 100
  ) {
    showError(
...
async function loadSavedForms() {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:5000/get-forms/${currentTrainer.id}${roleParam}`
    );
...
async function bulkDeleteSelected() {
  ...
  showConfirmModal(
    ...
    async () => {
      try {
        for (const id of ids) {
          await fetch(`http://localhost:5000/delete-form/${id}`, {
            method: "DELETE",
...
async function adminCloneForEdit(formId) {
  ...
  try {
    // 1) جلب الملف الأصلي
    const res = await fetch(`http://localhost:5000/get-form/${formId}`);
    ...
    const saveRes = await fetch("http://localhost:5000/save-form", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function editForm(formId) {
  try {
    const response = await fetch(`http://localhost:5000/get-form/${formId}`);
    const data = await response.json();
...
async function deleteForm(formId) {
  showConfirmModal(
    ...
    async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/delete-form/${formId}`,
          {
            method: "DELETE",
            headers: {
...
async function previewForm() {
  ...
  try {
    const response = await fetch("http://localhost:5000/preview-form", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
async function previewSavedForm(formId) {
  try {
    ...
    if (data.success) {
      ...
      const previewRes = await fetch("http://localhost:5000/preview-form", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          lessonName: form.lessonName,
    ...
    async () => {
      try {
        const url = currentFormId
          ? `http://localhost:5000/update-form/${currentFormId}`
...
async function saveEditForm() {
  ...
  showConfirmModal(
    ...
    async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/update-form/${editingFormId}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
...
async function confirmAddTrainer() {
  ...
  try {
    const res = await fetch("http://localhost:5000/add-trainer", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
async function handleFileUpload(input) {
  ...
  try {
    ...
    if (isImage) {
      ...
      // رفع الملف
      const response = await fetch("http://localhost:5000/upload-lesson-file", {
        method: "POST",
        body: formData,
      });
...
async function suggestQuestions() {
  ...
  try {
    ...
    const response = await fetch("http://localhost:5000/suggest-questions", {
      method: "POST",
      headers: headers,
      body: requestBody,
    });
...
async function suggestTrainerCredentials() {
  ...
  try {
    const name = nameEl?.value.trim() || "";
    const response = await fetch(
      "http://localhost:5000/suggest-trainer-credentials",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function fetchAndRenderTrainersList() {
  ...
  try {
    const response = await fetch("http://localhost:5000/all-trainers");
    const data = await response.json();
...
async function saveTrainerEdit(event) {
  ...
  try {
    const response = await fetch("http://localhost:5000/edit-trainer", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
